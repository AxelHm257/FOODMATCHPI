...
<body class="bg-gray-50">
    <div class="max-w-6xl mx-auto p-6">
        ...
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('admin.providers.create') }}" class="px-3 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">Nuevo proveedor</a>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Propietario</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($providers as $p)
                        <tr>
                            ...
                            <td class="px-4 py-2">{{ optional($p->user)->name }}</td>
                            <td class="px-4 py-2">{{ $p->contact }}</td>
                            <td class="px-4 py-2">{{ $p->location }}</td>
                            <td class="px-4 py-2">
                                <a href="{{ route('admin.providers.edit', $p) }}" class="px-3 py-1 border border-indigo-600 text-indigo-700 rounded hover:bg-indigo-50">Editar</a>
                                <form method="POST" action="{{ route('admin.providers.destroy', $p) }}" class="inline" onsubmit="return confirm('¿Eliminar proveedor?');">
                                    @csrf
                                    @method('DELETE')
                                    <button class="px-3 py-1 border border-red-600 text-red-700 rounded hover:bg-red-50">Eliminar</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
